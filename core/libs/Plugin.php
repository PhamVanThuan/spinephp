<?php
	...
	class Plugin {
		 ...
		 *
		 * @param mixed $plugin
		 * @param boolean $instatiate
		 * @return mixed
		 */
		public static function load($plugin, $instatiate = false){
			if(is_array($plugin)){
				list($file, $class) = $plugin;
			}else{
			...
			if(file_exists(BASE_PATH . 'plugins/' . $file . '.plugin.php') && !array_key_exists($class, Plugin::$loaded)){
				require_once(BASE_PATH . 'plugins/' . $file . '.plugin.php');
